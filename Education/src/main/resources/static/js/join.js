 ...
 */
	 		...
	 		const name = $('#name');
			const id = $('#id');
			const pwd = $('#hidden_pwd');
			const pwdCheck = $('#hidden_pwdCheck');
			const hp = $('#hp');
			const reg_no = $('#hidden_reg_no');
			const email = $('#email1');
			const join_day = $('#join_day');
			const zip = $('#zip');
			const years = $('#years');
			const salary = $('#salary');
				
			$(function () {
				$('#summit_btn').attr("disabled", true)
// 				필수항목 미입력시 등록버튼 비활성화 
				$('#name,#id,#hidden_pwd,#hidden_pwdCheck,#hp,#hidden_reg_no,#email1,#join_day').on('change', function () {
					...
					if(name.val()!=''&&id.val()!=''&&pwd.val()!=''&&pwdCheck.val()!=''&&hp.val()!=''&&reg_no.val()!=''&&email.val()!=''&&join_day.val()!=''){
...
// 				이메일 도메인 미선택시 도메인 비활성화
				$('#i_domain').attr("readonly", true).css("background", "aqua")
				$('#s_domain').on('input', function () {
...
// 				연령 비활성화
// 				$('#years').css("background", "aqua")
				
// 				사업자등록증,이력서
				if($('#cmp_reg_image, #carrier').val()==''){
					$('#cmp_reg_image, #carrier').css("background", "aqua").attr("readonly", true)
				}
...
// 				비밀번호 *처리
				$('#hidden_pwd').on("blur", function() {
					var hidden_pwd = $('#hidden_pwd').val();
					var pwd = $('#hidden_pwd').val();
					var pwdLast = pwd.charAt(pwd.length-1)
					var c = '';
					for(var i=0; i<hidden_pwd.length-1; i++){
						c += '*';
					}
					c += pwdLast;
					$('#hidden_pwd').val(c);
				...
				$('#hidden_pwdCheck').on("blur", function() {
					var hidden_pwdCheck = $('#hidden_pwdCheck').val();
					var pwdCheck = $('#hidden_pwdCheck').val();
					var hidden_pwdLast = pwdCheck.charAt(pwdCheck.length-1)
					var c = '';
					for(var i=0; i<hidden_pwdCheck.length-1; i++){
						c += '*';
					}
					c += hidden_pwdLast;
					$('#hidden_pwdCheck').val(c);
					$('#pwdCheck').val(pwdCheck);
...
// 			    profile preview
				...
				imgTarget.on('change', function(){
			        ...
			        if(window.FileReader){
			            ...
			            reader.onload = function(e){
			                var src = e.target.result;
			                parent.prepend('<div class="upload-display"><img src="'+src+'" style="width:100%; margin-top:10px;"></div>');
			            }
			            reader.readAsDataURL($(this)[0].files[0]);
			        }
...
// 			insert 페이지 등록
			function insert_summit() {
				...
				if($('#hidden_pwd').val().length<8||$('#hidden_pwd').val().length>16){
